...
from lac_interfaces.msg import ImagePair, StereoIMU
...
class AgentNode(Node):
    def __init__(self):
        ...
        self.front_stereo_publisher = self.create_publisher(ImagePair, "/front_stereo_img_pair", 10)
    ...
    def publish_true_pose(self, pose_array):
        pose = Pose()
        pose.position.x = pose_array[0]
        pose.position.y = pose_array[1]
        pose.position.z = pose_array[2]
        
        x, y, z, w = tf2_ros.transformations.quaternion_from_euler(pose_array[3:])
        pose.orientation.x = x
        pose.orientation.y = y
        pose.orientation.z = z
        pose.orientation.w = w
        
        self.true_pose_publisher.publish(pose)
...
class AgentV1(AutonomousAgent):
    ...
    def launch_nodes(self):
        """
        Launches all nodes related to the agent.
        """
        ld = launch.LaunchDescription([
            launch_ros.actions.Node(
                package='perception',
                executable="pc_generation_node",
                name='pc_generation_node',
                output="screen"
            ),
            launch_ros.actions.Node(
                package='perception',
                executable="pc_aggregator_node",
                name='pc_aggregator_node',
                output="screen"
            )
            # ADD MORE NODE DESCRIPTORS HERE
        ])
    ...
    def run_step(self, input_data):
        ...
        if front_stereo_img_data[0] is not None and front_stereo_img_data[1] is not None and true_pose is not None:
            pose_arr = np.array([
