...
class OpenCVagent(AutonomousAgent):
    ...
    def run_step(self, input_data):
        ...
        sensor_data_front_right = input_data['Grayscale'][carla.SensorPosition.FrontRight]
        
        sensor_data_back_left = input_data['Grayscale'][carla.SensorPosition.BackLeft]
        sensor_data_back_right = input_data['Grayscale'][carla.SensorPosition.BackRight]
        
        sensor_data_array = [sensor_data_front_left, sensor_data_front_right, sensor_data_back_left, sensor_data_back_right]
        ...
        outL = cv.VideoWriter('left_test_vid.mp4', fourcc, 20.0, (frame_width, frame_height))
        outR = cv.VideoWriter('right_test_vid.mp4', fourcc, 20.0, (frame_width, frame_height))
        ...
        for i, sensor_data in enumerate(sensor_data_array):
            if sensor_data is not None:
                ...
                if i == 0:
                    ...
                    cv.waitKey(1)
                if self.frame <= 200:
                    if i == 0: # write left test img
                        # cv.imwrite('perception/test_imgs/stereo_left_test.png', sensor_data_front_left)
                        outL.write(sensor_data_front_left)

                    if i == 1: # write right test img
                        # cv.imwrite('perception/test_imgs/stereo_right_test.png', sensor_data_front_right)
                        outR.write(sensor_data_front_right)
                    
                else:
                    outL.release()
                    outR.release()
                    self.mission_complete() # finish program once images are saved
