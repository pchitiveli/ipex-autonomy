...
from lac_interfaces.msg import ImagePair, EulerOrientation, PoseStampedImagePair
...
class PointCloudGenerator(Node):
    ...
    def __init__(self):
        # initialize node
        super().__init__("pc_generation_node")

        self.bridge = CvBridge()

        self.pos = np.array([0.0, 0.0, 0.0])  # None
        self.rpy = np.array([0.0, 0.0, 0.0])  # None

        # initialize local point cloud for storing PC
        self.pc = o3d.geometry.PointCloud()

        # initialize variables for opencv images
        self.imgL = None
        self.imgR = None
        ...
        self.front_img_pair_sub = self.create_subscription(
            PoseStampedImagePair, "/filtered_stereo_img_pair", self.front_img_pair_callback, 10
        )
        
        # create publishers
        self.pc_publisher = self.create_publisher(PointCloud2, "/point_cloud", 10)
    ...
    def front_img_pair_callback(self, msg):
        self.imgL = self.bridge.imgmsg_to_cv2(msg.image_pair.left, desired_encoding="mono8")
        self.imgR = self.bridge.imgmsg_to_cv2(msg.image_pair.right, desired_encoding="mono8")

        if self.imgL is not None and self.imgR is not None:
            print("[PC GEN] Front Image Pair Received")
            ...
            self.rpy[0] = msg.orientation.roll
            self.rpy[1] = msg.orientation.pitch
            self.rpy[2] = msg.orientation.yaw

            # sharpen left and right images to increase contrast and pc gen accuracy
            # already sharpened if subscribed to yolo filtered image topics
            # self.imgL = cv2.filter2D(self.imgL, -1, self.sharpen_kernel)
            # self.imgR = cv2.filter2D(self.imgR, -1, self.sharpen_kernel)

            # resize images and recalculate
            self.imgL = cv2.resize(
                self.imgL, (self.IMG_WIDTH, self.IMG_HEIGHT)
            )
            self.imgR = cv2.resize(
                self.imgR, (self.IMG_WIDTH, self.IMG_HEIGHT)
            )

            depth_image = self.compute_depth_image(self.imgL, self.imgR)

            o3d_pc = self.compute_point_cloud_from_depth(
                self.imgL, depth_image, origin_pos=self.pos, origin_rpy=self.rpy, voxel_size=0.03
            )
            # o3d_pc, _ = o3d_pc.remove_radius_outlier(nb_points=2, radius=0.2)

            self.pc_publisher.publish(
                self.o3d_pc_to_point_cloud2(
                    self.get_clock().now().to_msg(), o3d_pc
                )
            )
